...
const CategoryNav = () => {
    ...
    return (
        <div className="bg-[#FDF5F6] border-b border-[#EBCDD0] hidden md:block sticky top-[77px] z-40 shadow-sm font-sans">
                        ...
                        >
                            <Link
                                ...
                                className={`font-display text-sm tracking-[0.15em] font-semibold flex items-center gap-1 transition-all duration-300 relative py-2
                                    ${hoveredCategory === cat.id ? 'text-[#D39A9F]' : 'text-black'}
                                `}
                            >
                                {cat.name}
                                <span className={`absolute bottom-0 left-0 w-full h-[2px] bg-[#D39A9F] transform transition-transform duration-300 ${hoveredCategory === cat.id ? 'scale-x-100' : 'scale-x-0'}`}></span>
                            </Link>
                            ...
                            <AnimatePresence>
                                {hoveredCategory === cat.id && (
                                    <motion.div
                                        ...
                                        className="absolute left-0 top-full w-full bg-white shadow-2xl border-t border-[#EBCDD0] py-12 min-h-[400px]"
                                    >
                                                            ...
                                                            <div key={subCat.id} className="flex items-center gap-4 group">
                                                                ...
                                                                <div className="flex flex-col justify-center">
                                                                    <Link to={`/category/${subCat.path}`} onClick={() => setHoveredCategory(null)}>
                                                                        <h4 className="font-display font-bold text-black text-lg group-hover:text-[#D39A9F] transition-colors">{subCat.name}</h4>
                                                                    </Link>
                                                                    <Link to={`/category/${subCat.path}`} onClick={() => setHoveredCategory(null)} className="text-[10px] font-bold text-[#D39A9F] uppercase tracking-wider mt-1 hover:underline opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-1 group-hover:translate-y-0">
                                                                        View Collection
                                                                    </Link>
                                                                </div>
                                                            </div>
                                                ...
                                                <div className="flex gap-12">
                                                    ...
                                                    >
                                                        <div>
                                                            ...
                                                            <Link to={`/category/${cat.path}`} onClick={() => setHoveredCategory(null)} className="inline-flex items-center text-xs font-bold uppercase tracking-widest text-[#D39A9F] hover:text-black transition-colors group">
                                                                View All Products <MoveRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                                                            </Link>
                                                        </div>
                                                        ...
                                                        <ul className="space-y-3">
                                                            {cat.subcategories?.map((sub, idx) => (
                                                                <li key={idx}>
                                                                    <Link to={`/category/${cat.path}/${sub.name.toLowerCase()}`} onClick={() => setHoveredCategory(null)} className="text-gray-600 hover:text-[#D39A9F] hover:tracking-wide transition-all duration-300 font-medium flex items-center justify-between group">
                                                                        {sub.name}
                                                            ...
                                                            {cat.subcategories?.slice(0, 4).map((sub, index) => (
                                                                <Link key={index} to={`/category/${cat.path}/${sub.name.toLowerCase()}`} onClick={() => setHoveredCategory(null)} className="group block relative overflow-hidden rounded-lg">
                                                                    <div className="aspect-[3/4] overflow-hidden bg-gray-100 relative">
                                                                        <div className="absolute inset-0 bg-[#D39A9F]/0 group-hover:bg-[#D39A9F]/10 z-10 transition-colors duration-500" />
                                                                        <img
                                                                            src={sub.image}
                                                                            alt={sub.name}
